...
(function () {
  ...
  const THEMES = new Set(["signature", "signature-white", "fedex", "cream"]);
  const THEME_COLORS = {
    signature: "#1a0088",
    "signature-white": "#ffffff",
    fedex: "#4d148c",
    cream: "#f4f1ec",
  };
  const THEME_INFO = {
    signature: {
      title: "Original",
      desc: "The original theme — navy #1A0088 with orange #FF5E32 accents.",
      dot: "linear-gradient(135deg, #1a0088 60%, #ff5e32 60%)",
    },
    "signature-white": {
      title: "Original + white bg",
      desc: "The original theme on a white page background; the footer and dark accent bands stay navy for contrast.",
      dot: "linear-gradient(135deg, #ffffff 55%, #1a0088 55%)",
    },
    fedex: {
      title: "FedEx",
      desc: "The previous brand theme — purple #4D148C with orange #FF6600.",
      dot: "linear-gradient(135deg, #4d148c 60%, #ff6600 60%)",
    },
    cream: {
      title: "Logo Colours",
      desc: "The logo palette — #F4F1EC paper with blue #1A0088 and orange #FF5E32.",
      dot: "linear-gradient(135deg, #f4f1ec 50%, #1a0088 50%)",
    },
  };
  ...
  function showThemeToast(theme) {
    ...
    toastEl.innerHTML =
      '<span class="theme-toast__dot" style="background:' + info.dot + '"></span>' +
      '<span><p class="theme-toast__title">' + info.title + "</p>" +
      '<p class="theme-toast__desc">' + info.desc + "</p></span>";

    // Force reflow so re-triggering the animation works on repeated clicks.
  ...
  function applyTheme(theme) {
    const activeTheme = THEMES.has(theme) ? theme : DEFAULT_THEME;
  ...
  function initThemeSwitcher() {
    ...
    switchers.forEach((switcher) => {
      switcher.querySelectorAll("[data-theme-option]").forEach((button) => {
        button.addEventListener("click", () => {
          const nextTheme = THEMES.has(button.dataset.themeOption) ? button.dataset.themeOption : DEFAULT_THEME;
