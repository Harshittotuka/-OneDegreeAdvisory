@php
    ...
    $titles = ['dashboard' => ['Dashboard', 'Your lead performance at a glance'], 'leads' => ['Leads', 'Website and manually created enquiries in one place'], 'enrollments' => ['Website payments', 'Payment records classified by program and source page'], 'subscriptions' => ['Subscriptions', 'Manage newsletter subscriptions in one simple list'], 'followups' => ['Follow-up planner', 'Every open conversation, upcoming and overdue'], 'students' => ['Enrolled students', 'Track students through admissions and visa stages'], 'journeys' => ['Journey planners', 'Every student whose journey planner has started, and how far along they are'], 'audit' => ['Audit log', 'See every CRM action and who performed it'], 'spam' => ['Blocked submissions', 'Public-form submissions caught by the honeypot field'], 'shortlisting' => ['PDF shortlisting', 'Replace a report PDF\'s last page with a university shortlist'], 'mock-invites' => ['Mock interviews', 'Issue extended mock-interview links and see how students scored'], 'team' => ['Team management', 'Create counsellors, super admins and referral partners, and control their CRM access'], 'partner-codes' => ['Partner codes', 'Referral companies and the tracking link each of their codes gives them']];
    $currentTitle = $titles[$view];
    $isListView = ! in_array($view, ['dashboard', 'enrollments', 'subscriptions', 'audit', 'spam', 'shortlisting', 'team', 'partner-codes', 'journeys'], true);
    ...
    // Leads-table "Partner" cell. Two separate facts share the column: the
    // partner ACCOUNT the lead was pointed at (who can see it), and the partner
    // CODE its link carried (where it came from). A lead can have either, both,
    // or neither, so both are rendered rather than one standing in for the other.
    // Trusted markup, rendered with {!! !!}.
    $partnerCell = static function (\App\Models\CrmLead $lead): string {
        $parts = [];
        if ($lead->partner) {
            $parts[] = '<span class="lead-partner">'.e($lead->partner->name).'</span>';
        }
        if ($lead->partnerCode) {
            $parts[] = '<span class="lead-partner-code" title="Referred through code '.e($lead->partnerCode->code).'">'
                .e($lead->partnerCode->company_name).'</span><span class="subtext">Code '.e($lead->partnerCode->code).'</span>';
        }
        return $parts === [] ? '<span class="subtext">—</span>' : implode('', $parts);
    };
...
<div class="shell">
    <aside class="sidebar" id="sidebar">
        ...
        <nav class="nav">
            ...
            @unless($crmUser->isPartner())<a class="nav-link nav-journeys {{ $view === 'journeys' ? 'active' : '' }}" href="{{ route('crm.dashboard', ['view' => 'journeys']) }}"><span class="nav-icon">@include('crm.partials.nav-icon',['name'=>'journeys'])</span><span class="nav-text">Journey planners</span>@if($journeyCount)<span class="nav-badge">{{ $journeyCount }}</span>@endif</a>@endunless
    ...
    <main class="main">
        ...
        <div class="content">
            ...
            @elseif($view === 'journeys')
                @include('crm.partials.journeys')
            ...
            @elseif($view === 'partner-codes')
                @include('crm.partials.partner-codes')
            @else
            <section class="workspace">
                <form id="crmLeadFilters" @class(['filters', 'lead-classification-filters' => $isListView]) method="get" action="{{ route('crm.dashboard') }}" data-crm-filter-form>
                    ...
                    @endif
                    {{-- And the same for the referral code the lead arrived through.
                         "No partner code" is the direct traffic. --}}
                    @if($partnerCodeOptions->isNotEmpty())
                        @include('crm.partials.multi-filter', [
                            'name' => 'partner_code_id',
                            'options' => ['none' => 'No partner code'] + $partnerCodeOptions->mapWithKeys(fn ($code) => [(string) $code->id => $code->label()])->all(),
                            'selected' => \App\Support\CrmFilter::raw(request(), 'partner_code_id'),
                            'placeholder' => 'Every partner code', 'label' => 'Filter by partner code', 'noun' => 'codes',
                        ])
                    @endif
                    @if($crmUser->isSuperAdmin())
                        ...
                        @if($view === 'leads')
                            ...
                            @foreach($leads as $lead)
                                ...
                                <tr data-crm-href="{{ $openUrl }}" tabindex="0">
                                    ...
                                    @unless($crmUser->isPartner())<td>{!! $partnerCell($lead) !!}</td>@endunless
                        ...
                        @else
                            ...
                            @foreach($leads as $lead)
                                ...
                                <tr data-crm-href="{{ $openUrl }}" tabindex="0">
                                    ...
                                    @if($crmUser->isSuperAdmin())<td>@if($lead->assignee)<span class="owner"><span class="avatar">{{ $initials($lead->assignee->name) }}</span>{{ $lead->assignee->name }}</span>@else<span class="subtext">Unassigned</span>@endif</td>@endif @unless($crmUser->isPartner())<td>{!! $partnerCell($lead) !!}</td>@endunless
    ...
    <div class="modal">
        ...
        <form method="post" action="{{ route('crm.leads.store') }}">@csrf
            <div class="modal-body"><div class="form-grid">
                ...
                @if($partners->isNotEmpty())
                    @php
                        $createdPartner = $partners->firstWhere('id', (int) old('partner_id'));
                ...
                <form method="post" action="{{ route('crm.leads.update',$selectedLead) }}" data-lead-details-form data-track-changes
                    ...
                    <div class="drawer-card section-card">
                        ...
                        <div class="form-grid roomy-grid">
                            ...
                            @unless($crmUser->isPartner())<div class="field"><label>Partner name <span class="label-note">Who referred this student</span></label><select name="partner_id" data-partner-select><option value="">No partner</option>@foreach($partners as $partner)<option value="{{ $partner->id }}" data-code-id="{{ $partner->partnerCode?->id }}" data-code-label="{{ $partner->partnerCode?->label() }}" @selected($selectedLead->partner_id===$partner->id)>{{ $partner->name }}@unless($partner->is_active) (disabled)@endunless</option>@endforeach</select></div>@endunless
                    ...
                    <div class="drawer-card section-card">
                        ...
                        <div class="form-grid roomy-grid">
                            ...
                            <div class="field"><label>Email address <span class="label-note">Optional</span></label><input type="email" name="email" value="{{ $selectedLead->email }}" placeholder="student@domain.com"></div><div class="field"><label>Current city <span class="label-note">Optional</span></label><input name="city" value="{{ $selectedLead->city }}" placeholder="e.g. Jaipur"></div>
